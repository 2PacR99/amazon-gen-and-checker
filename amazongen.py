...
def amazonapp():
    init(convert=True)
    ctypes.windll.kernel32.SetConsoleTitleW("Amazon Gen and Checker Made With Python By S2Market")
    ...
    if option == '1':
        ...
        for i in range(amount):
            ...
            f.close()
            fix += 1
            ctypes.windll.kernel32.SetConsoleTitleW("Generated Codes: " + str(fix) + "/" + str(amount) + " | MADE BY S2Market")
        time.sleep(2)
        os.system('cls')
        amazonapp()
    ...
    if option == '2':
        giftcards = []
        num = 0
        speed = (1/int(input("How Many Codes Per Second? [RECOMMENDED: 4]: ")))
        ...
        def checker():
            try:
                ...
                r = requests.post("https://www.amazon.com/gc/redeem?ref_=gcui_b_e_r_c_d_b_w", data={"giftcard": kll})
